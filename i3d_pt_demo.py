...
def run_demo(args):
    ...
    def get_scores(sample, model):
        ...
        out = model(sample_var)
        out_logit = torch.mean(out,dim=2)
        out_var = F.softmax(out_logit,dim=1)
    ...
    if args.flow and args.rgb:
        ...
        for i in range(args.top_k):
            logit_score = out_logit[0, top_idx[0, i]].item()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--rgb_weights_path',
        type=str,
        default='models/rgb_imagenet.pt',
        help='Path to rgb model state_dict')
    parser.add_argument(
        '--rgb_sample_path',
        type=str,
        default='data/v_CricketShot_g04_c01_rgb.npy',
    ...
    parser.add_argument(
        '--flow_weights_path',
        type=str,
        default='models/flow_imagenet.pt',
        help='Path to flow model state_dict')
    parser.add_argument(
        '--flow_sample_path',
        type=str,
        default='data/v_CricketShot_g04_c01_flow.npy',
